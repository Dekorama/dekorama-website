 ...
 * @param {{ variant?: 'spain' | 'caracas', hasServerLcp?: boolean }} props
 */
export default function Hero({ variant = 'spain', hasServerLcp = false }) {
  ...
  const serverShell = hasServerLcp

  // Interactive chrome only when server already painted LCP shell
  if (serverShell) {
    ...
    return (
      <>
        ...
        <div className="absolute bottom-5 left-1/2 z-20 flex -translate-x-1/2 gap-1 pb-[env(safe-area-inset-bottom,0px)] sm:bottom-6">
          {slides.map((_, i) => (
            <button
              key={i}
              type="button"
  ...
  return (
    ...
    >
      <div className={`absolute inset-0 ${index === 0 ? 'opacity-100' : 'opacity-0'}`}>
        <Image
          src={first.image}
          alt={index === 0 ? first.alt : ''}
          fill
          priority
          fetchPriority="high"
          decoding="sync"
