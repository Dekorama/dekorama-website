...
export default function ContactChannels({
  ...
  const phoneReady = market.phoneReady

  return (
    <div
      className={`flex flex-col sm:flex-row items-center justify-center gap-8 ${className}`}
    >
      {showWhatsApp && phoneReady && market.whatsappUrl ? (
        <a
        ...
        >
          ...
          <span className="font-medium">{market.phoneDisplay}</span>
        </a>
      ) : null}
      ...
      >
        ...
        <span className="font-medium">{market.email}</span>
      </a>
    </div>
  )
}
