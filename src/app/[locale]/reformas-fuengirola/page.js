...
export async function generateMetadata({ params }) {
  ...
  const t = await getTranslations({ locale, namespace: 'ciudades.fuengirola' })
  ...
  return {
    ...
    openGraph: {
      title: t('title'),
      description: t('description'),
      url: `/${locale}/reformas-fuengirola`,
    },
    alternates: { 
      canonical: `${baseUrl}/${locale}/reformas-fuengirola`,
      languages: {
        'es': `${baseUrl}/es/reformas-fuengirola`,
        'en': `${baseUrl}/en/reformas-fuengirola`,
      }
    },
  }
}

export default async function ReformasFuengirolaPage({ params }) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'ciudades.fuengirola' })
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      },
      {
        '@type': 'ListItem',
        position: 2,
        name: locale === 'es' ? 'Reformas Fuengirola' : 'Renovations Fuengirola',
        item: `${baseUrl}/${locale}/reformas-fuengirola`,
      },
    ],
  }
          ...
          <div className="max-w-7xl mx-auto">
            <nav className="text-sm text-gray-500 mb-8">
              ...
              <span className="text-black font-medium">
                {locale === 'es' ? 'Reformas Fuengirola' : 'Renovations Fuengirola'}
              </span>
            </nav>
            ...
            <div className="grid md:grid-cols-2 gap-12 items-center">
              ...
              <div className="relative aspect-[4/3] rounded-lg overflow-hidden">
                <Image
                  src={images.services.cocinas}
                  alt="Reformas integrales Fuengirola - Dekorama"
                  fill
